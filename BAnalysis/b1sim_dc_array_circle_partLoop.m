...
% purposes.
function b1sim_dc_array_circle_partLoop(r,ny,nx,nz,fm_dim,cy,cx,cz, START, STOP)

curr_path = fileparts(mfilename('fullpath'));
cd(curr_path)

addpath('../biot_savart_solver_code')
output_stem='dc_array_circle8';

field_map_matrix     = [ny nx nz];  % number of field maps points along x, y, and z
voxel_offset = fm_dim./field_map_matrix/2;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
N     = [cx(2:2:end),cy(2:2:end),cz(2:2:end)];
T     = zeros(4,4);
circle_radius = repmat(r,[ports, 1]);
...
for p = 1:ports
    ...
    for d = 1:div
        ...
        if d < START || d > STOP  %FINAL: d < 18+35 || d > 33+24 INITIAL: d > -10 || d < div+10 % Introduce a gap in the loop
            cc2               = T*[circle{d}.stop 1]';          %
            current{p,d}.stop = cc2(1:3)';                      %
        else                                                    %
            cc2               = T*[circle{d}.start 1]';         %
            current{p,d}.stop = cc1(1:3)'+[1e-10 1e-10 1e-10];  %
        end;                                                    %
        % cc2=T*[circle{d}.stop 1]';        
        % current{p,d}.stop=cc2(1:3)';
    end;
...
%figure(3) % generate Brainmesh figure
...
for p = 1:ports
    for d = 1:div
        ...
        set(h,'color',[0 0 1]), set(h,'linewidth',3);
    end;
end;
grid on;
axis tight equal;
...
dim_x_start = -fm_dim(1)/2 + voxel_offset(1); 
dim_x_stop  = fm_dim(1)/2 - voxel_offset(1);
dim_x_mat   = field_map_matrix(1);
dim_x_skip  = (dim_x_stop-dim_x_start)./(dim_x_mat-1);
dim_x       = [dim_x_start:dim_x_skip:dim_x_stop];

dim_y_start = -fm_dim(2)/2 + voxel_offset(2);
dim_y_stop  = fm_dim(2)/2 - voxel_offset(2);
dim_y_mat   = field_map_matrix(2);
dim_y_skip  = (dim_y_stop-dim_y_start)./(dim_y_mat-1);
dim_y       = [dim_y_start:dim_y_skip:dim_y_stop];

dim_z_start = -fm_dim(3)/2 + voxel_offset(3) + 20; % arbitrary 20 mm offset for a specific application
dim_z_stop  = fm_dim(3)/2 - voxel_offset(3) + 20;
...
%%%%%%%%%%%%%%%%%% formating b1 %%%%%%%%%%%%%%%%%%%%
figure(100)
slice(fov_x,fov_y,fov_z,b1_effect_total,[],[],[]); %z = [1] plane
colormap(gray);
...
%%%%%%%%%%% save data %%%%%%%%%%%%%%
save b1_1ch_save.mat b1_x b1_y b1_z fov_x fov_y fov_z current fov_grid